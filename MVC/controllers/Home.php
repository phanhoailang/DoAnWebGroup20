...
class Home extends Controller
{
  ...
  // danh sách sản phẩm đang được bán chạy, danh sách danh mục và tên người dùng
  function Default()
  {
    // session_destroy();
  ...
  //gắn kết quả cho $showNumAjax


 
 //lưu id sản phẩm vào session và hiển thị trang chi tiết sản phẩm
  function product($id)
  {
    if (isset($_SESSION['userlogin'])) {
      $user_id = $_SESSION['userlogin'][3];
  ...
  //tạo các đối tượng cho việc tương tác với cơ sở dữ liệu và hiển thị trang tin tức với danh sách tin tức.
  function newS()
  {
    if (isset($_SESSION['userlogin'])) {
      $user_id = $_SESSION['userlogin'][3];
  ...
  //tạo các đối tượng cho việc tương tác với cơ sở dữ liệu và hiển thị trang liên hệ với tên người dùng.
  function Contac()
  {
    if (isset($_SESSION['userlogin'])) {
      $user_id = $_SESSION['userlogin'][3];
  ...
  //hiển thị trang hồ sơ người dùng, dựa trên thông tin được lấy từ UserModel
  function Profile(){
    {
      if (isset($_SESSION['userlogin'])) {
        $user_id = $_SESSION['userlogin'][3];
  ...
  //hiển thị trang đăng nhập và lấy thông tin người dùng và danh mục liên quan từ cơ sở dữ liệu.
  function login()
  {
    if (isset($_SESSION['userlogin'])) {
      $user_id = $_SESSION['userlogin'][3];
...
//hiển thị trang đăng ký và lấy thông tin danh mục
  function regsiter()
  {
    $Category = $this->model("CategoryModel");
    $this->view("master3", [
  ...
  // hiển thị trang danh sách sản phẩm và lấy thông tin liên quan từ cơ sở dữ liệu để hiển thị trên trang
  function thucdon()
  {
  ...
  //kiểm tra và xử lý thanh toán, bao gồm cả kiểm tra giỏ hàng và trạng thái đăng nhập của người dùng
  function checkout()
  {
    if (isset($_SESSION['userlogin'])) {
      $user_id = $_SESSION['userlogin'][3];
  ...
  //hiển thị danh sách comment của sản phẩm và thực hiện kiểm tra 
